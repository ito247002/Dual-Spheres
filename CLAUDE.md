...
- `PLAYER_CONFIGS` - プレイヤーごとの個性（速度、連射間隔、弾数、弾速、扇形角度）
- `ITEM_TYPES` - アイテム定義（拡張可能）
- `COLORS` - パステルカラー定義（ピンク vs ミント）
...
### 追加したい項目
- プレイヤーの動作制御
    - あたかも人がプレイしているような動作を目指す。
    - プレイヤーは常に相手プレイヤーを狙う。
    - プレイヤーは常に弾を回避しようとするが、近くにアイテムがある場合はアイテムを優先して取得しにいく。



### プレイヤー個性（PLAYER_CONFIGS）
| 特性 | Player 1（ピンク） | Player 2（ミント） |
|------|-------------------|-------------------|
| タイプ | スピード型 | パワー型 |
| 移動速度 | 6 | 2.5 |
| 連射間隔 | 500ms | 800ms |
